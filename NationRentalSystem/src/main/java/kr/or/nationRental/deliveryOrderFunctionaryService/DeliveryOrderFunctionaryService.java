...
public class DeliveryOrderFunctionaryService {
	...
	public Map<String, Object> selectListDeliveryOrderFunctionary(int currentPage, int pagePerRow, String searchSelect, String searchWord, DeliveryOrderFunctionaryDto deliveryOrderFunctionaryDto){
		logger.debug("selectListDeliveryOrderFunctionary DeliveryOrderFunctionaryService");
		 ...
		 * */
		List<DeliveryOrderFunctionaryDto> list = deliveryOrderFunctionaryDao.selectListDeliveryOrderFunctionary(map);
		logger.debug("list<BoardDto> : " + list);
		/* 검색을 하였다면 검색조건에 맞는 레코드 개수가 반환되고,
		 * 검색을 하지 않았다면 DB에 존재하는 모든 address 레코드 개수가 반환된다. 
		 *  */
		int total = deliveryOrderFunctionaryDao.totalCountDeliveryOrderFunctionary(map);
